define(['widget/twitter'],function(){
	/********************************
	Tweets
	*********************************/
	...
	return new Class ({
		...
		**/
		render: function(data){
			...
			setInterval(function(){
				...
				if(numTweet >= cont.length - 1) numTweet = 0
					else numTweet++
		...
		**/
		parseTweetTextDate: function(date_text){
			...
			var now = new Date()
			var tdate = new Date(date_arr[2] + " " + date_arr[1] + ", " + date_arr[3] + " " + date_arr[4] + " UTC")
			...
			if (tdate.getDate() < (now.getDate() - 1)) {
				...
				if (tdate.getDate() < now.getDate()) { //tdate.getDate() == (now.getDate()-1)
					return "Yesterday" /* can you change for your language */
				} else {
		...
		renderTweet:function(tweet){
			...
			tweet = tweet.replace(/#\w+/g, function(data){ return '<span>'+data+'</span>'})
			tweet = tweet.replace(/https?:\/\/\w+\.\w+\/\w+/g, function(data){ return '<a href='+data+' target="_blank">'+data+'</a>'})
			return tweet;
		}
